...
public class DeliveryTxn implements Transaction {
  ...
  public void run(Session session, ItemsMetadata itemsMetadata) {
    for (int district_id = 1; district_id <= 10; ++district_id) {
      // Get the latest id
      String getNextOrderIdQuery = String.format("select D_NEXT_O_ID, D_LAST_UNDELIVERED_O_D from CS4224H.district_by_warehouse where w_id = %s and d_id = %s;", this.warehouse_id, district_id);
      ...
      String getCustomer = String.format("SELECT * FROM CS4224H.customers WHERE DUMMY_KEY = 1 AND C_W_ID = %s AND C_D_ID = %d and C_ID = %d;\n",
      ...
      String updateNextUndeliveredOrder = String.format("UPDATE CS4224H.district_by_warehouse SET D_LAST_UNDELIVERED_O_D = %d WHERE W_ID = %s AND D_ID = %d;", last_undelivered_order_id + 1, this.warehouse_id,
          district_id);     
      ...
      String updateOrdersByCustomer = String.format(
          ...
          this.carrier_id, formattedDate, this.warehouse_id, district_id, customer_id, last_undelivered_order_id);
