...
public class PaymentTxn implements Transaction {
  ...
  public void run(Session session, ItemsMetadata itemsMetadata) {
    ...
    String updateDistrictWarehouse = String.format(
        "UPDATE CS4224H.district_by_warehouse SET D_YTD = D_YTD + %f WHERE W_ID = %s AND D_ID = %s;",
        this.payment.doubleValue(), this.warehouse_id, this.district_id);

    // update customer
    String updateCustomer = String.format(
        "UPDATE CS4224H.customers SET C_BALANCE = C_BALANCE - %f, C_YTD_PAYMENT = C_YTD_PAYMENT + %f, C_PAYMENT_CNT = C_PAYMENT_CNT + 1 WHERE C_W_ID = %s AND C_D_ID = %s AND C_ID = %s;",
        this.payment.doubleValue(), this.payment.doubleValue(), this.warehouse_id, this.district_id, this.customer_id);
    ...
    System.out.printf(
        ...
        customer.getDecimal("C_CREDIT_LIM"), customer.getDecimal("C_DISCOUNT"), customer.getDecimal("C_BALANCE"));
