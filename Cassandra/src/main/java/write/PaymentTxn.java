...
public class PaymentTxn implements Transaction {
        ...
        public void run(Session session, ItemsMetadata itemsMetadata) {
                ...
                String getCustomerQuery = String.format(
                                "SELECT * FROM CS4224H.customers WHERE DUMMY_KEY = 1 AND C_W_ID = %s AND C_D_ID = %s AND C_ID = %s;",
                ...
                double new_c_ytd_payment = customer.getDecimal("C_YTD_PAYMENT").doubleValue()
                ...
                String updateCustomer = String.format(
                                "UPDATE CS4224H.customers SET C_BALANCE = %f, C_YTD_PAYMENT = %f, C_PAYMENT_CNT = %d WHERE DUMMY_KEY = 1 AND C_W_ID = %s AND C_D_ID = %s AND C_ID = %s;",
                                new_c_balance,
                                new_c_ytd_payment,
                                new_c_payment_cnt,
                                this.warehouse_id, this.district_id,
                ...
                System.out.printf(
                                ...
                                this.warehouse_id,
                                this.district_id, this.customer_id, customer.getString("C_FIRST"),
                                customer.getString("C_MIDDLE"),
                                customer.getString("C_LAST"), customer.getString("C_STREET_1"),
                                customer.getString("C_STREET_2"),
                                customer.getString("C_CITY"), customer.getString("C_STATE"),
                                customer.getString("C_ZIP"),
                                customer.getString("C_PHONE"), customer.getTimestamp("C_SINCE"),
